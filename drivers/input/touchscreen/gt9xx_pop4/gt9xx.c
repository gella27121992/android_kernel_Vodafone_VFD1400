...
int gtp_send_cfg(struct i2c_client *client)
{
    ...
    if (ts->fixed_cfg)
    {
        printk("Ic fixed config, no config sent!");
        return 0;
    }
...
static int gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    printk("CTP_CONFIG_GROUP%d used, config length: %d\n", sensor_id + 1, ts->gtp_cfg_len);
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            {
                printk("Ic fixed config with config version(%d, 0x%02X)\n", opr_buf[0], opr_buf[0]);
                ts->fixed_cfg = 1;
                gtp_get_info(ts);
                return 0;
            }
...
static int gtp_check_product_id(struct i2c_client *client)
{
	...
	ret = strncmp(product_id, ts->pdata->product_id,4);
	if (ret != 0)
		return -EINVAL;
...
printk("%s:====gpio irq: %d ====\n",__func__,gpio_to_irq(pdata->irq_gpio));
