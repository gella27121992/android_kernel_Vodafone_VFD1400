...
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2014.10.17 */
#ifdef CONFIG_TCT_8X26_TINT8
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, GTP_MAX_WIDTH - x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, GTP_MAX_HEIGHT - y);
#else
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
#endif
/* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2014.10.17 */
...
//[FEATURE]-Modify by TCTNB.TianHongwei 2014/5/19 gesture fuction.
	if (!strncmp(buf, "on", 2)) {
		ts->gesture_onoff = 1;
		device_init_wakeup(&i2c_connect_client->dev, 1);//[DEBUG]Modify by TCT-NB Tianhongwei 06/10/2014 PR.697258 gesture wake up is not very sensitive.
	} else if (!strncmp(buf, "off", 3)) {
		ts->gesture_onoff = 0;
		device_init_wakeup(&i2c_connect_client->dev, 0);//[DEBUG]Modify by TCT-NB Tianhongwei 06/10/2014 PR.697258 gesture wake up is not very sensitive.
	}
...
static DEVICE_ATTR(gesture, 0664, gtp_gesture_data_show, gtp_gesture_option_store);

static void gtp_gesture_attr_create(void)
{
   gtp_gesture_class = class_create(THIS_MODULE, "TP-UNLOCK");
    ...
    if (IS_ERR(gtp_gesture_dev))
       ...
       // update
    if (device_create_file(gtp_gesture_dev, &dev_attr_gesture) < 0)
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (DOZE_ENABLED == doze_status)
    {
        ...
        if (ret > 0)
        {     
		/*[DEBUG] modify by TCTNB tianhongwei 05/28/2014 app can't get the gesture point info sometimes*/
		  ...
		  else if(doze_buf[2] == 0x3E)
		  {
                ...
                input_sync(ts->input_dev);
/*[DEBUG] remove by TCTNB tianhongwei 05/28/2014 app can't get the gesture point info sometimes*/
                // clear 0x814B
                doze_buf[2] = 0x00;
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
			...
			goodix_power_off(ts);
#endif
	    /* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2014.12.11, reset = 0 while sleeping */
	    GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);
	    msleep(5);
	    /* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2014.12.11 */
            return ret;
        }
        msleep(10);
    }
...
/* [PLATFORM]-MODIFIED-BEGIN by TCTSZ.lizhi.wu, 2014.12.11 */
    //while(retry++ < 5)
    //{
...
/* [PLATFORM]-REMOVED-BEGIN by TCTSZ.lizhi.wu, 2014.11.11 ,for TP's power on timer list
    gtp_reset_guitar(ts->client, 10);
 [PLATFORM]-REMOVED-ENDby TCTSZ.lizhi.wu, 2014.11.11 ,for TP's power on timer list*/

/* [PLATFORM]-REMOVED-BEGIN by TCTSZ.lizhi.wu, 2014.11.11 ,for TP's power on timer list*/
    GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);
    GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
    msleep(10);
/* [PLATFORM]-REMOVED-END by TCTSZ.lizhi.wu, 2014.11.11 ,for TP's power on timer list*/
...
#if GTP_ICS_SLOT_REPORT
    input_mt_init_slots(ts->input_dev, 5);  //[debug] TCT-NB tianhongwei . TP may not work after andriod reboot.in case of "out of memory"
#else
...
//[DEBUG] MOdify TCT-NB tianhongwei
	if (!IS_ERR(ts->vcc_i2c)) {
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2014.10.3 , for tint8's I2C voltage is lvs1, the type is RPM_REGULATOR_SMD_TYPE_VS, so the n_voltages = 0, if use regulator_set_voltage ,it will be wrong*/
		if (regulator_count_voltages(ts->vcc_i2c) > 0) {
/* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2014.10.3 */
...
static int goodix_power_init(struct goodix_ts_data *ts)
{
	...
	ts->vcc_i2c = regulator_get(&ts->client->dev, "vcc-i2c");/*[BUGFIX]-MOD  by TCTNB.XQJ,09/18/2013,FR-514195,GT915 tp MIATA development,i2c,only for compil*/
	if (IS_ERR(ts->vcc_i2c)) {
...
static DEVICE_ATTR(debug, 0664, NULL, gtp_debug_option_store);
/*[FEATURE]-Add-end by TCTNB.TianHongwei*/
...
/* [PLATFORM]-Add-BEGIN by TCTSZ.lizhi.wu, 2014.11.11 ,for TP's power on timer list*/
...
/* [PLATFORM]-Add-END by TCTSZ.lizhi.wu, 2014.11.11 ,for TP's power on timer list*/
...
/* [PLATFORM]-ADD-BEGIN by TCTSZ.lizhi.wu, 2014.12.10 */
    if (gpio_is_valid(GTP_INT_PORT))
	gpio_free(GTP_INT_PORT);
    if (gpio_is_valid(GTP_RST_PORT))
	gpio_free(GTP_RST_PORT);
    kfree(ts);
/* [PLATFORM]-ADD-END by TCTSZ.lizhi.wu, 2014.12.10 */
    return ret;
...
//    sysfs_remove_group(&(ts->input_dev->dev.kobj), &gtp_gesture_attribute_group);
    device_remove_file(gtp_gesture_dev, &dev_attr_gesture);
#endif
    goodix_power_off(ts);
    goodix_power_deinit(ts);
...
//[DEBUG]-Add-end by TCTNB.TianHongwei 2014/5/19
    if (!ts->gtp_is_suspend) {
		GTP_DEBUG("Already in awake state.");
		return 0;
    }
