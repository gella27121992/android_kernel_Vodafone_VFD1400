...
int fts_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
    ...
    if(readlen > 0) {
        if (writelen > 0) {
            ...
            for (count = 1; count <= FTS_I2C_RETRY_TIMES; count++) {
                ret = i2c_transfer(client->adapter, msgs, 2);
                if (ret == 2)
                    break;
                dev_err(&client->dev, "%s: i2c read error, try times %d!\n",
                        __func__, count);
            }
        } else {
            struct i2c_msg msgs[] = {
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
    ...
    if(writelen > 0) {
        for (count = 1; count <= FTS_I2C_RETRY_TIMES; count++) {
            ...
            if (ret == 1)
                break;
            dev_err(&client->dev, "%s: i2c write error, try times %d!\n",
                    __func__, count);
        }
    }
