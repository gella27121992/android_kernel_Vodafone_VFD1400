...
CSID_TG:
  ...
  if(s_ctrl->sensordata->power_info.gpio_conf->gpio_num_info->valid[SENSOR_GPIO_ID] == 1) {
   err = gpio_request(s_ctrl->sensordata->power_info.gpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_ID] ,
    "CAM_CTYPE");
 
    if(err){pr_err("request gpio camera id failed\n");}
    	
    err = gpio_direction_input(s_ctrl->sensordata->power_info.gpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_ID]);
    
    if(err){pr_err("change camera id direction failed\n");}
  }
