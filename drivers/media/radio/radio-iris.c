				...
				enum iris_evt_t event)
{
	...
	//FMDERR("radio %p event %d", radio, event);
		...
		void *param)
{
	...
	struct sk_buff *skb;
	int ret = 0;
	//FMDERR("hdev %p opcode %u len %u", hdev, opcode, plen);
						...
						bool interruptible)
{
	...
	//FMDERR("hdev %p cmd 0x%x", hdev, cmd);
...
{
	struct kfifo *data_b;
	//FMDERR("radio %p data %p len %d event %d", radio, data, len, event);
...
static int set_low_power_mode(struct iris_device *radio, int power_mode)
{
	...
	//FMDERR("power mode old 0x%x new 0x%x", radio->power_mode, power_mode);
			...
			struct v4l2_ext_controls *ctrl)
{
	...
	//FMDERR("0x%x", ctrl->controls[0].id);
			...
			struct v4l2_ext_controls *ctrl)
{
	...
	case V4L2_CID_RDS_TX_PS_NAME:
		FMDERR("In V4L2_CID_RDS_TX_PS_NAME\n");
	...
	case V4L2_CID_PRIVATE_IRIS_WRITE_DEFAULT:
		...
		FMDERR("XFR Mode\t: 0x%x", default_data.mode);
		FMDERR("XFR Data Length\t: %d\n", default_data.length);
		/*
	...
	case V4L2_CID_PRIVATE_IRIS_STATE:
		...
		case FM_RECV:
			if (radio->mode != FM_OFF) {
				FMDERR("%s: FM is not in proper state %d\n", __func__, radio->mode);
				retval = -EINVAL;
				goto END;
			}
