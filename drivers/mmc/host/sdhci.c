...
static u8 sdhci_calc_timeout(struct sdhci_host *host, struct mmc_command *cmd)
{
	 ...
	 * timeout value.
	 */
	//[PLATFROM]-Mod-BEGIN by SCDTABLET.jinghuang@tcl.com,04/12/2016,1940590
	//recovery fail ,mmc erase timeout:modify erase way from secdiscard to discard.
	#ifdef CONFIG_TCT_8937_POP4104GVDF
	return 0xF;
	#endif
	//[PLATFROM]-Mod-END by SCDTABLET.jinghuang@tcl.com
...
void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	timeout = jiffies;
	//[BUGFIX]-Mod-BEGIN by SCDTABLET.jinghuang@tcl.com,03/24/2016,1856900
	//factory crash
	//[BUGFIX]-Mod-BEGIN by SCDTABLET.jinghuang@tcl.com,03/07/2016,1753039,
	//add timer to 30s for cmd38 timeout
	#ifdef CONFIG_TCT_8937_POP4104GVDF
	if (!cmd->data && cmd->busy_timeout > 9000)
		timeout += DIV_ROUND_UP(cmd->busy_timeout*2, 1000) * HZ + HZ;
	else
		timeout += 30 * HZ;
	#else
	if (!cmd->data && cmd->busy_timeout > 9000)
