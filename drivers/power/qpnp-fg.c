...
static struct fg_mem_setting settings[FG_MEM_SETTING_MAX] = {
	...
	SETTING(SOFT_HOT,        0x454,   1,      650),
	SETTING(HARD_COLD,       0x454,   2,      -300),
	SETTING(HARD_HOT,        0x454,   3,      700),
#else
...
struct fg_chip {
	...
	bool			charge_full;
/*[PLATFORM]-MOD-BEGIN by huichen@tcl.com, trace to 100% issue, 20160505 alm 2075686 */
	bool			charge_keep_full;
...
/*[PLATFORM]-MOD-BEGIN by huichen@tcl.com, trace to 100% issue, 20160505 alm 2075686 */
	if (chip->status == POWER_SUPPLY_STATUS_FULL && capacity !=100 ) {
		if(!chip->charge_keep_full)
			{
			chip->charge_keep_full=1;
			}
		pr_err("charge_keep_full=1\n");
	capacity = get_prop_capacity(chip);
		}
	else if(chip->status != POWER_SUPPLY_STATUS_FULL)
		{
		chip->charge_keep_full=0;
		pr_err("charge_keep_full=0\n");
		}
/*[PLATFORM]-MOD-END by huichen@tcl.com*/
	...
	if (chip->status == POWER_SUPPLY_STATUS_FULL) {
/*[PLATFORM]-MOD-BEGIN by huichen@tcl.com, trace to 100% issue, 20160428 alm 1991115 */
