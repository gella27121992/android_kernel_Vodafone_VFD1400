...
// [FEATURE]-ADD-BEGIN huichen@tcl.com,2016/03/17,alm 1748743
#if defined(CONFIG_TCT_8937_POP4104GVDF)
	struct delayed_work		jeita_work;
	bool temp_up;
	struct wakeup_source	jeita_work_ws;
#endif
// [FEATURE]-ADD-END huichen@tcl.com,2016/03/17
/* [PLATFORM]-Mod-BEGIN by huichen@tcl.com, 2016/03/22, add battery status debug message alm 1849154*/
#if (defined CONFIG_QPNP_BATT_STATUS_DEBUG) && defined( CONFIG_TCT_8937_POP4104GVDF)
	int index;
	struct delayed_work		monitor_workqueue;
	struct wakeup_source	monitor_wake_ws;
#endif
...
{
//[PLATFORM]-MOD-BEGIN by huichen@tcl.com, for mmitest, 20160217 ALM 1546529
...
//[PLATFORM]-MOD-BEGIN by huichen@tcl.com, for OVP, 20160318 ALM 1839414
//#define USBIN_ADAPTER_5V_UNREGULATED_9V	0x5
#define USBIN_ADAPTER_5V_UNREGULATED_9V	0x1
//[PLATFORM]-MOD-END by huichen@tcl.com
...
//       schedule_delayed_work(&chip->jeita_work,msecs_to_jiffies(JEITA_WORK_MS));
    #endif
// [FEATURE]-ADD-END huichen@tcl.com,2016/03/17
...
/* [PLATFORM]-Mod-BEGIN by huichen@tcl.com, 2016/03/22, add battery status debug message alm 1849154*/
#if (defined CONFIG_QPNP_BATT_STATUS_DEBUG) && defined( CONFIG_TCT_8937_POP4104GVDF)
		__pm_stay_awake(&chip->monitor_wake_ws);
#endif
/* [PLATFORM]-Mod-END by huichen@tcl.com */
...
/* [PLATFORM]-Mod-BEGIN by huichen@tcl.com, 2016/03/22, add battery status debug message alm 1849154*/
#if (defined CONFIG_QPNP_BATT_STATUS_DEBUG) && defined( CONFIG_TCT_8937_POP4104GVDF)
		__pm_relax(&chip->monitor_wake_ws);
#endif
/* [PLATFORM]-Mod-END by huichen@tcl.com */
...
#if defined(CONFIG_TCT_8937_POP4104GVDF)
	chip->temp_up=true;
	__pm_stay_awake(&chip->jeita_work_ws);
    cancel_delayed_work_sync(&chip->jeita_work);
	schedule_delayed_work(&chip->jeita_work,msecs_to_jiffies(JEITA_WORK_MS));
#endif
// [FEATURE]-ADD-END huichen@tcl.com,2016/03/17
...
enum thermal_mitigation_current{
        THERMAL_MITIGATION_550   = 550,
        THERMAL_MITIGATION_1000  = 1000,
        THERMAL_MITIGATION_1500  = 1500,
        THERMAL_MITIGATION_1800  = 1800,
	THERMAL_MITIGATION_2000  = 2000,
};

static void pmi8952_jeita_work_fn(struct work_struct *work)
{

   struct smbchg_chip *chip = container_of(work,struct smbchg_chip,jeita_work.work);
#ifdef TARGET_BUILD_MMITEST
#else
   int temp;
   bool batt_warm = false, batt_cold = false, batt_hot = false, batt_cool = false;
   int low_temp=0, hi_temp=0;
   temp = get_prop_batt_temp(chip);
   pr_err("chenhui pmi8952_jeita_work_fn temp=%d\n",temp);
if(chip->temp_up==true)
{
   pr_err("chenhui temp up\n");
   if (temp >= 510){
			/* Warm to hot */
			pr_err("chenhui Warm to hot \n");
	  batt_hot=true;
	  batt_warm=false;
	  batt_cool=false;
	  batt_cold=false;
...
{
   pr_err("chenhui temp down\n");
   if(temp<=40)
   {
   /* Cool to cold */
			pr_err("chenhui Cool to cold\n");
	  batt_hot=false;
	  batt_warm=false;
	  batt_cool=false;
	  batt_cold=true;
...
//	power_supply_changed(&chip->batt_psy);
	pr_err("cold = %d, hot = %d, warm = %d, cool = %d\n",
		chip->batt_cold ^ batt_cold, chip->batt_hot ^ batt_hot, chip->batt_warm ^ batt_warm, chip->batt_cool^batt_cool);
	if (chip->batt_cold ^ batt_cold || chip->batt_hot ^ batt_hot  || chip->batt_warm ^ batt_warm || chip->batt_cool^batt_cool)
     {
		if (batt_cold || batt_hot) {
			pr_err("chenhui disable charge\n");
			chip->batt_cold = batt_cold;
		chip->batt_cool = batt_cool;
		chip->batt_warm = batt_warm;
			chip->batt_hot = batt_hot;
		...
		schedule_work(&chip->usb_set_online_work);
			...
			if (chip->batt_cold ^ batt_cold || chip->batt_hot ^ batt_hot) {
			//need enable charge
		vote(chip->usb_suspend_votable, USER_EN_VOTER,
				0, 0);
		vote(chip->dc_suspend_votable, USER_EN_VOTER,
		...
		schedule_work(&chip->usb_set_online_work);
			...
			pr_err("chenhui limit voltage and current %d\n",batt_warm?THERMAL_MITIGATION_1000:THERMAL_MITIGATION_2000);
			smbchg_float_voltage_set(chip, 4400);
			if(batt_warm)
      smbchg_set_fastchg_current_raw(chip,THERMAL_MITIGATION_1000);
			else
	  smbchg_set_fastchg_current_raw(chip,THERMAL_MITIGATION_2000);
	...
	set_property_on_fg(chip,POWER_SUPPLY_PROP_COOL_TEMP,low_temp);
	set_property_on_fg(chip,POWER_SUPPLY_PROP_WARM_TEMP,hi_temp);
#endif
	__pm_relax(&chip->jeita_work_ws);

	//pm_relax(chip->dev);
	pr_err("chenhui pmi8952_jeita_work_fn end\n");
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	if (chip->safety_time != -EINVAL) {
		...
		if (rc < 0)
			...
			{
			chip->safety_timer_en = true;
// [FEATURE]-ADD-BEGIN huichen@tcl.com,2016/03/29,alm 1863398
#if defined(CONFIG_TCT_8937_POP4104GVDF) 
			smbchg_safety_timer_enable(chip, false);
#endif
// [FEATURE]-ADD-END huichen@tcl.com,2016/03/29
			}
	}
	...
	if (rc < 0)
		dev_err(chip->dev, "Couldn't read usb allowance rc=%d\n", rc);
//[PLATFORM]-MOD-BEGIN by huichen@tcl.com, for OVP, 20160318 ALM 1839414
	rc = smbchg_masked_write(chip, 	chip->usb_chgpth_base + USBIN_CHGR_CFG,0x7,
		(chip->original_usbin_allowance & 0x03));
	if (rc < 0)
		dev_err(chip->dev, "Couldn't write usb allowance rc=%d\n", rc);
	rc = smbchg_read(chip, &chip->original_usbin_allowance,
			chip->usb_chgpth_base + USBIN_CHGR_CFG, 1);
...
static void qpnp_chg_monitor_work_callback(struct work_struct *work)
{
	...
	struct smbchg_chip *chip = container_of(dwork,struct smbchg_chip,monitor_workqueue);
...
//			struct qpnp_lbc_chip, monitor_workqueue);
	...
	int get_current = get_prop_batt_current_now(chip);

	chip->index++;
	do_gettimeofday(&now);
	rtc_time_to_tm(now.tv_sec, &tm);
	pr_err("BMS_DEBUG %-8d    %02d:%02d:%02d    %8d    %3d%%    %3dC   %d\n",chip->index, tm.tm_hour, tm.tm_min, tm.tm_sec, voltage, capacity, temperature,get_current);
    if(temperature>=570 || temperature <=-170)
	power_supply_changed(&chip->batt_psy);
//	power_supply_changed(&chip->batt_psy);
...
// [FEATURE]-ADD-BEGIN huichen@tcl.com,2016/03/17,alm 1748743
#if defined(CONFIG_TCT_8937_POP4104GVDF)
    INIT_DELAYED_WORK(&chip->jeita_work, pmi8952_jeita_work_fn);
    wakeup_source_init(&chip->jeita_work_ws, "smbcharge_jeita_work_ws");
#endif
// [FEATURE]-ADD-END huichen@tcl.com,2016/03/17
/* [PLATFORM]-Mod-BEGIN by huichen@tcl.com, 2016/03/22, add battery status debug message alm 1849154*/
#if (defined CONFIG_QPNP_BATT_STATUS_DEBUG) && defined( CONFIG_TCT_8937_POP4104GVDF)
	INIT_DELAYED_WORK(&chip->monitor_workqueue, qpnp_chg_monitor_work_callback);
	wakeup_source_init(&chip->monitor_wake_ws, "smbcharge_monitor_wake_ws");
#endif
/* [PLATFORM]-Mod-END by huichen@tcl.com */
